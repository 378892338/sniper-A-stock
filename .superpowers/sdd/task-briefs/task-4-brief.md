# Task 4: MootdxRealtimeSource — 通达信 TCP 实时行情

**Files:**
- Create: `data/sources/mootdx_realtime.py` (将 MootdxRealtimeSource 放在独立文件，与现有 mootdx.py 分开)
- Modify: `data/sources/__init__.py` — 注册新源
- Test: `tests/test_mootdx_realtime.py`

## P0 修复覆盖
- P-CRIT-01: 北交所过滤 (8xxxxx/920xxx)
...
Create `data/sources/mootdx_realtime.py`:
...
In `data/sources/__init__.py`, add the import and registration at the end:
...
Create `tests/test_mootdx_realtime.py`:
...
```bash
git add data/sources/mootdx_realtime.py data/sources/__init__.py tests/test_mootdx_realtime.py
git commit -m "feat(mootdx): add MootdxRealtimeSource with IP pool, rate limit, BJ filter, vol÷100"
```
